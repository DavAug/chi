...
problem.fix_parameters({
    ...
    'Absorption rate in 1/d': 55,  # Data not rich enough
    'Plasma conc. in ng/mL Sigma base': 0.001,   # Regularises Error
    'Tumour volume in cm^3 Sigma base': 0.001})  # Regularises Error
...
log_priors = [
    ...
    pints.TruncatedGaussianLogPrior(
        mean=1, sd=1, a=0, b=np.inf),        # Sigma Initial tumour volume
    pints.TruncatedGaussianLogPrior(
    ...
    pints.TruncatedGaussianLogPrior(
        mean=0.1, sd=0.1, a=0, b=np.inf),    # Mu Potency
    pints.TruncatedGaussianLogPrior(
        mean=0.1, sd=0.1, a=0, b=np.inf),    # Sigma Potency
    pints.TruncatedGaussianLogPrior(
        mean=0.1, sd=1, a=0, b=np.inf),      # Mu Growth rate
    pints.TruncatedGaussianLogPrior(
        mean=1, sd=1, a=0, b=np.inf),        # Sigma Growth rate
    pints.TruncatedGaussianLogPrior(
