...
class MarginalPosteriorPlot(eplt.MultiSubplotFigure):
    ...
    .. note::
        The convergence metric :math:`\hat{R}` is computed for each marginal
        posterior. The computed metric will only be correct if samples are
        ordered according to their sample iteration, e.g as returned by the
        :meth:`SamplingController.run`.
